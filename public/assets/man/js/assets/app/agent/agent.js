define(["jquery", "alertify", "amui", "pjax", "common", "chosen", "handlebars"], function ($, alertify, AMUI, pjax, common, chosen, Handlebars) {
	...
	$(function(){
		_init();
		$main = $("#admin-content");
		$main.delegate("#agent-rate-alert", "click", function(){
			alertify.alert(this.dataset.text);
		});
/**
 *	运营商信息修改弹窗
 * */
		$main.delegate('.system-agent-edit', 'click', function(){
			...
			$.get(
				...
				function(response){
					...
					{
						$("#popup-sub-title").text(response.data.ent_name)
						...
						var _min_val = Math.round(parseFloat(response.data.gains.default_lvl) * 100) / 100;
						$("#box_min_rate").val(_min_val);
						$("#span_min_rate").text(_min_val);
						var _max_val = _min_val;
						var _my_val = 0;
						if ( response.data.my_rate != false )
						{
							_my_val = Math.round(parseFloat(response.data.my_rate.rate_lvl) * 100) / 100;
						}
						$("#box_default_rate").val(_my_val);
						_max_val = _min_val + _my_val;
						$("#span_max_rate").text(_max_val);
						var _rate_valid = common.parseValidator($("#box_gain_rate").data("validation"));//.replace("{{#min_val#}}", _min_val).replace("{{#max_val#}}", _max_val);
						_rate_valid.minValue = _min_val;
						_rate_valid.maxValue = _max_val;
						$("#box_gain_rate").data("validation", common.JSON.encode(_rate_valid));
 ...
 * */
		$main.delegate("#agent_grid_page a, #btn_agent_search", "click", function(e){
			...
			var _page = ( e.currentTarget.tagName == 'BUTTON') ? 1 : $(this).data("ciPaginationPage"),
				_get_url = '/index.php/agent/agent/list/' + ( parseInt(_cate_id) > 0 ? _cate_id : 0 ) + "/" + _page;
